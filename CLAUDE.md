...
- `TASK_PREFETCH_IMAGES` (`prefetch-images`) warms the artwork cache for a server. Enqueued by the `sync-server` task **after a successful full sync** (`syncMediaServer` throws on failure, so a failed sync never schedules one), with jobKey `prefetch-images:<serverId>` and **no queue name** — it is slow, entirely best-effort, and must not sit on `MAIN_QUEUE` blocking the next sync/lifecycle/backup job. Deliberately **not** enqueued from the incremental sync: that fires on every realtime `library-changed` event, and a full artwork scan per event would cost far more than the handful of new items justify (they warm on the next full sync and stay lazy until then).
