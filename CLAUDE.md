...
- `src/app/(authenticated)/` — All protected pages. Auth enforced by layout.tsx which validates session and redirects if invalid.
  ...
  - Library: `/library/movies`, `/library/series` (with `/seasons` and `/episodes` sub-routes), `/library/music`. The series `show`/`season`/`episode` detail pages each render `SeriesWatchHistory` (`src/components/series-watch-history.tsx`) over `GET /api/media/series/watch-history` — the per-play list (user / date-time / episode) for the whole series, one season, or one episode. On the show/season pages it is a full-width section; on the episode page it is passed as `MediaDetailContent`'s `historySection` slot (`variant="card"`), which **replaces** the built-in per-user aggregate Watch History card and skips that card's live `/api/media/[id]/history` fetch — an episode gets one history section, not the same plays twice. Movies and tracks keep the built-in aggregate card. That route matches the series on **`seriesKey`** — the **same series-identity key `/api/media/series/grouped` groups shows by** (see "Series identity" below) — so two genuinely different shows that share a title stay separate, while the same show on two servers still merges (shared TVDB-derived key) and keeps every server's plays. It accepts a `seriesKey` query param (preferred; the detail pages pass `item.seriesKey`) and still falls back to the legacy `parentTitle` param (matched case/space-insensitively) when no key is given. It resolves the in-scope episode ids first, then filters `WatchHistory` by `mediaItemId`, which also keeps the history query on the `WatchHistory(mediaItemId)` index. Every user's plays are included — `session.userId` is the Librariarr admin who owns the server record, not a media-server account, and nothing filters on `serverUsername`. It reads the stored `WatchHistory` table and deliberately does **not** apply dedup: a row is a real play event recorded against the copy that was actually played, so filtering to `dedupCanonical` would drop every play that happened on a non-canonical server. `/api/media/[id]/history` answers a different question (per-user aggregate for ONE item, fetched live from the server, no per-play timestamps) and is not a substitute — it still backs the history card for movies and tracks
