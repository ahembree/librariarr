...
- `src/lib/events/event-bus.ts` + `src/app/api/events/stream/route.ts` + `src/hooks/use-realtime.ts` — the **browser push layer**. `eventBus` is an in-process EventEmitter; `/api/events/stream` subscribes to it, filters by `session.userId`, and re-emits over SSE; `useRealtime(type, cb)` subscribes a component. **Every `AppEventType` must have at least one `useRealtime` subscriber** — asserted by `tests/unit/events/event-subscriber-coverage.test.ts`, which exists because `server:changed` and `settings:changed` were each emitted by several routes, forwarded correctly, and read by **nobody**: a dangling emitter is silent by construction, so the only way to catch one is to assert the two halves against each other. Add a `SERVER_ONLY` entry (with a reason) if an event genuinely has no browser consumer. **Events carry no figures** — the receiver refetches the route that owns the number, so there is exactly one place each value is computed; this is why `sync:progress`, `session-changed` and `watch-history:updated` all ship `meta.serverId` and nothing else. **`watch-history:updated` is deliberately NOT `sync:completed`**: that event has 16 subscribers that refetch whole media listings, and a bulk history import can commit for a long time, so reusing it would re-pull tens of thousands of rows repeatedly (`src/lib/sync/watch-history-events.ts` documents the split). `sync:progress` is throttled 2s per server in `src/lib/sync/sync-progress.ts` and its throttle is **cleared on sync end** so the next run's first batch isn't swallowed by the previous run's window.
- **The shared `EventSource` is ref-counted, and the ref count must never reach zero on a navigation.** `useRealtime` shares one connection per tab; React unmounts the outgoing page's subscribers *before* mounting the incoming page's, so a page-level-only subscriber set makes `refCount` hit 0 on **every** route change. Closing immediately there produced measured connection lifetimes of **1–17s against the 1h design** (13 connections in a 180s window on the dev instance), and every event emitted in those gaps was lost — there is no replay, since the route sends no event ids so `Last-Event-ID` cannot be honoured either. Two things prevent it: `releaseSource()` defers the close by `IDLE_CLOSE_DELAY_MS` (10s), cancelled by the next `acquireSource()`; and `AuthenticatedShell` holds a subscription, so a subscriber always exists. **Do not remove the shell's `useRealtime` call** — it is load-bearing for the transport, not just for the maintenance banner. On a *re*-open the hook fires every subscriber's callback once (`fireResync`), because callbacks are refetches and refetching is the only way to recover a gap. A 401 is distinguished from a dropped connection by a `HEAD` probe against the same route (the route exports an explicit `HEAD` — without it Next.js derives HEAD from GET and every probe would open a real stream) and stops the retry loop instead of silently retrying an expired session forever.
- **Polls that remain are fallbacks, deliberately slow.** SSE dies to proxy buffering and idle timeouts, so every converted poll was *slowed*, not deleted: sidebar sessions 30s→5min, maintenance 30s→5min, sync progress 2s→15s. Do not "clean up" a fallback poll by removing it. The one poll that is the sole update path by design is `/api/tools/sessions/stream`'s own 5s server-side tick, which reconciles what the socket may have missed.
- `src/lib/media-server/realtime/` — Real-time media-server WebSocket layer. One self-healing connection **per enabled server** (keyed by `MediaServer.id`, so multiple servers of the same type each connect), started from `instrumentation.ts` via `startRealtime()`. Per-type normalizers (`normalize-plex.ts`, `normalize-jellyfin.ts` — the latter shared by Jellyfin+Emby) collapse server messages into canonical `RealtimeEvent`s (`session-changed`/`library-changed`/`watch-changed`/`server-status`) on an in-process `realtimeBus` (separate from the app `eventBus`). `connection.ts` owns connect→open→message→close→reconnect with exponential backoff (1s→60s) + keepalive, and de-duplicates the Jellyfin/Emby `Sessions` firehose (a fixed-period frame every ~1.5s) via a position-free signature so only real changes surface; `manager.ts` reconciles the live connection set against the DB (honoring the `AppSettings.realtimeSync` toggle; concurrent reconciles coalesce) and routes events to rate-limited side effects: a **leading-edge-throttled** enforcer tick (session — immediate on a real change, floored to once/2s so a push loop can't drive getSessions-on-all-servers), a **debounced** `TASK_SYNC_INCREMENTAL` (library — the manager accumulates the changed/removed ratingKeys the event carried and applies just those items, falling back to a full `TASK_SYNC_SERVER` when the change set is >100, unmappable, or item-less), and a **debounced** `TASK_SYNC_WATCH_HISTORY` (watch). Uses the `ws` package; the socket is injected (`SocketFactory`) so the connection/manager are unit-testable without real sockets. Reconciles on `server:changed`/`settings:changed` app events (emitted by the server CRUD + `/api/settings/realtime` routes) plus a 60s safety pass. The sessions SSE stream (`/api/tools/sessions/stream`) subscribes to `realtimeBus` for instant pushes (throttled), falling back to its 5s poll when realtime is off. `GET /api/tools/realtime/status` exposes per-server connection state. **Two of its events are bridged to the app `eventBus`** so they reach the browser: `server-status` (only on a `connected`↔`disconnected` transition — `connecting` is skipped, or the shell's unreachable banner would flash on every reconnect attempt) and `session-changed` (throttled 2s per server, so the sidebar's stream badge is pushed instead of polling `/api/tools/sessions`, which round-trips to every media server). `RealtimeServerConfig` carries `userId` for this reason — the bridge addresses a browser event on the socket's hot path and must not hit the DB per event. Nothing else crosses: `realtimeBus` also carries the per-1.5s Jellyfin session firehose
