...
**Deletion ceiling** (`AppSettings.maxAutoDeleteItems`, migration `0016_max_auto_delete_items`, policy in `src/lib/lifecycle/delete-ceiling.ts`): an OPT-IN limit on how many items one destructive run may act on. `null` (the default) means unlimited, and that default is deliberate — Librariarr exists to delete media unattended, and a user whose rules legitimately select thousands of items must not be second-guessed for it. A non-positive stored value reads as `null` rather than as a literal ceiling, since 0 almost certainly means "off" and taking it literally would silently disable lifecycle deletion entirely. What it defends against is not a rule being wrong but something UPSTREAM of the rule being wrong: every vacuous-match guard here refuses a hazard someone already found, and a ceiling is the only defence that does not require having enumerated them. Counted per RUN across all rule sets, over `isDestructiveActionType` actions only (unmonitor/tag/search runs are unaffected), and it holds the WHOLE batch rather than trimming to the limit — acting on an arbitrary subset of a match set you already distrust is not safer, just quieter. Enforced on all three bulk paths: `executeLifecycleActions` (grouped per user, after the stale-match/exception/item-existence filtering so the count is what would actually be destroyed; blocked actions stay PENDING and untouched, so the Pending page's existing Execute button IS the manual approval), `POST /api/lifecycle/actions/execute`, and `POST /api/query/actions` — the last one matters because the client batches around `MAX_QUERY_ACTION_ITEMS`, so that per-request cap never bounded a large selection. A held run notifies over Discord when configured, because a ceiling that silently does nothing is worse than none: the actions sit PENDING with no reason for the user to look.
