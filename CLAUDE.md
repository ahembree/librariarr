...
- **Ordering**: `DELETION_DATE` sort pools the PENDING `LifecycleAction` rows of **all** contributing rule sets, so deletion-date order is correct across rules. Collection sync therefore runs **after** action scheduling (`processLifecycleRules` after its loop; the manual `runDetection` path via `syncCollectionsAfterDetection` called from `/api/lifecycle/rules/run` after `scheduleActionsForRuleSet`).
