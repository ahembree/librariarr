...
- `TASK_SYNC_WATCH_HISTORY` (`sync-watch-history`) refreshes **only** a server's watch history (no full library re-scan) and invalidates media caches. Enqueued (debounced, on `MAIN_QUEUE`, jobKey `watch-history:<serverId>`) by the realtime manager on a `watch-changed` event; skips itself if a full sync is already running/queued for that server (the full sync already refreshes watch history). The realtime manager likewise enqueues durable `TASK_SYNC_SERVER` jobs (reusing the scheduler's `sync:<serverId>` jobKey) on `library-changed` events.
