...
- `src/lib/media-server/realtime/` — Real-time media-server WebSocket layer. One self-healing connection **per enabled server** (keyed by `MediaServer.id`, so multiple servers of the same type each connect), started from `instrumentation.ts` via `startRealtime()`. Per-type normalizers (`normalize-plex.ts`, `normalize-jellyfin.ts` — the latter shared by Jellyfin+Emby) collapse server messages into canonical `RealtimeEvent`s (`session-changed`/`library-changed`/`watch-changed`/`server-status`) on an in-process `realtimeBus` (separate from the app `eventBus`). `connection.ts` owns connect→open→message→close→reconnect with exponential backoff (1s→60s) + keepalive, and de-duplicates the Jellyfin/Emby `Sessions` firehose (a fixed-period frame every ~1.5s) via a position-free signature so only real changes surface; `manager.ts` reconciles the live connection set against the DB (honoring the `AppSettings.realtimeSync` toggle; concurrent reconciles coalesce) and routes events to rate-limited side effects: a **leading-edge-throttled** enforcer tick (session — immediate on a real change, floored to once/2s so a push loop can't drive getSessions-on-all-servers), a **debounced** `TASK_SYNC_INCREMENTAL` (library — the manager accumulates the changed/removed ratingKeys the event carried and applies just those items, falling back to a full `TASK_SYNC_SERVER` only when the change set is **>100**; an **item-less** `library-changed` now contributes NOTHING rather than forcing a full sync. That escalation was the single most expensive line in the app: `normalize-plex` turned every ended `library.*` activity into an id-less event and the manager latched a sticky `forceFull` for the whole debounce window, so the real ratingKeys collected beside it were discarded. Measured against a live server — adding one movie emitted dozens of `timeline` frames and thousands of `activity` frames, and cost **two** whole-server syncs; Plex's periodic `library.refresh.items` refresh (Context null, zero timeline entries, i.e. nothing changed) cost a full server sync *each time it fired*. `normalize-plex` no longer emits `library-changed` for `activity` **at all** — `timeline` is the only source, because it is the only channel naming *which* items changed, and adds, updates AND deletions all appear there (verified on the wire: an added movie `state=1 metadataState=created`, a deleted movie `sectionID=1 type=1 state=9`, a deleted episode `sectionID=2 type=4 state=9`). Ignoring `activity` wholesale is also what makes this future-proof: a new Plex activity type is ignored automatically, while a new item change still arrives via timeline. The scheduled full sync stays the reconciliation backstop), and a **debounced** `TASK_SYNC_WATCH_HISTORY` (watch). Uses the `ws` package; the socket is injected (`SocketFactory`) so the connection/manager are unit-testable without real sockets. Reconciles on `server:changed`/`settings:changed` app events (emitted by the server CRUD + `/api/settings/realtime` routes) plus a 60s safety pass. The sessions SSE stream (`/api/tools/sessions/stream`) subscribes to `realtimeBus` for instant pushes (throttled), falling back to its 5s poll when realtime is off. `GET /api/tools/realtime/status` exposes per-server connection state
