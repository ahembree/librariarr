...
export function BreakdownChart({
  ...
  const allSorted: SortedEntry[] = Array.from(aggregated.entries()).sort((a, b) => {
    const totalA = a[1].movies + a[1].series + a[1].music;
    const totalB = b[1].movies + b[1].series + b[1].music;
  ...
  // Build hex color map — use provided hexColors or auto-assign
  const hexMap: Record<string, string> = {};
  for (let i = 0; i < sorted.length; i++) {
    const label = sorted[i][0];
    if (label === "Other" && !hexColors?.[label]) {
      hexMap[label] = OTHER_HEX;
    } else {
      hexMap[label] = hexColors?.[label] ?? AUTO_HEX[i % AUTO_HEX.length];
    }
  }
  ...
  // Pie chart data (fill embedded in data for recharts v3)
  const pieData = visible.map(([label, counts]) => {
    ...
    return { name: label, value: count, pct: total > 0 ? (count / total) * 100 : 0, fill: hexMap[label] ?? OTHER_HEX };
  }).filter((d) => d.value > 0);
                        ...
                        <Tooltip key={label}>
                          <TooltipTrigger asChild>
                            <div
                              ...
                              style={{ width: `${pct}%`, backgroundColor: hexMap[label] ?? OTHER_HEX }}
                            />
                          </TooltipTrigger>
                ...
                <tr key={label} className="border-b last:border-0">
                  <td className="px-4 py-2 flex items-center gap-2">
                    <div
                      ...
                      style={{ backgroundColor: hexMap[label] ?? OTHER_HEX }}
                    />
                    {label}
                  </td>
                  {effectiveType ? (
