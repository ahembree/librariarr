  ...
  if ((url || apiKey) && enabled !== false) {
    ...
    if (!result.ok) {
      return NextResponse.json(
        { error: "Failed to connect to Sonarr", detail: sanitizeErrorDetail(result.error) },
        { status: 400 }
      );
    }
  }

  const instance = await prisma.sonarrInstance.update({
    where: { id },
    data: {
      ...(name && { name }),
      ...(url && { url: url.replace(/\/+$/, "") }),
...
export async function DELETE(
  ...
  await prisma.sonarrInstance.delete({ where: { id } });
