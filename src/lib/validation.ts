...
// Tautulli (Plex monitoring/stats). Tied to a Plex MediaServer via mediaServerId.
export const tautulliInstanceCreateSchema = z.object({
  name: z.string().min(1, "Name is required"),
  url: z.url("Invalid URL format"),
  apiKey: z.string().min(1, "API key is required"),
  mediaServerId: z.string().min(1).optional(),
});

export const tautulliInstanceUpdateSchema = tautulliInstanceCreateSchema.partial().extend({
  enabled: z.boolean().optional(),
});
