...
4. Paste the webhook URL (once saved it is shown masked — a webhook URL lets anyone post to the channel, so the app never returns it again; paste a new one to replace it)
