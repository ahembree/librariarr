...
Each sync replaces the server's **native** history with the latest data from the server. History enriched from [Tautulli](/docs/integrations/tautulli/) is preserved across native syncs and refreshed separately.
...
### Richer History with Tautulli

Connecting a [Tautulli](/docs/integrations/tautulli/) instance to a Plex server adds session detail Plex's own history API doesn't expose — the **transcode decision**, the **source→delivered stream** comparison (codec, resolution, bitrate, dynamic range, HW transcoding), client **IP/location**, and **pause/percent-complete** engagement. Tautulli plays are correlated to existing Plex history rows and merged where they line up; plays with no match are kept as their own rows, so nothing is lost.
