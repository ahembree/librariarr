...
Over 20 filters are available. Filters can be combined — multiple active filters apply with AND logic.
...
<Aside>
  Duplicates are detected by comparing title, year, and season/episode info. In rare cases, different items with identical metadata may be incorrectly grouped.
</Aside>
