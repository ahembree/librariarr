...
Detection therefore refuses to evaluate a rule set whose *arr or Seerr fields have no enabled instance behind them. The rule set is skipped with a warning in System Logs, its existing matches are left untouched, and previews return an error rather than a whole-library match list. An unreachable instance is skipped for that cycle for the same reason. Seerr fields are rejected outright on music rule sets, where Seerr data can never exist; a music rule set that already carries them is **disarmed** — matches cleared, pending actions cancelled.

The same refusal applies to ad-hoc **Query page actions**: running an action on results whose Arr/Seerr criteria couldn't actually be evaluated (no Arr server selected for the action's media type, no enabled Seerr instance selected, or Seerr criteria on music) is blocked with an explanatory error before anything runs.
