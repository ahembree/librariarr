...
There's a related trap on the outside-system side itself: fields like `foundInArr` and `seerrRequested` describe an item's **absence** from the external system, so if the external data simply isn't there — you disabled or deleted your only Radarr instance, or never connected Seerr — "not found in Arr" would be true for *everything*. Detection therefore refuses to evaluate a rule set whose *arr/Seerr fields have **no enabled instance** behind them: the rule set is skipped with a warning in System Logs, its existing matches are left untouched, and previews return an error instead of a whole-library match list. An unreachable (but still enabled) instance behaves the same way for a different reason — the metadata fetch fails and the rule set is skipped for that cycle. Seerr fields are additionally rejected outright on music rule sets, where no Seerr data can ever exist — and a legacy music rule set that already carries them is **disarmed**, not just skipped: its stale matches are cleared and pending actions cancelled (with an executor-side backstop), so a vacuous whole-library match armed before this guard existed can never execute.
