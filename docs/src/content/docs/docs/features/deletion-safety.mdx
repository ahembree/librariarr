...
| Claim | Where to look |
|-|-|
| Empty / disabled rule sets can't match | `hasAnyActiveRules` in `src/lib/rules/engine.ts` (early return at the top of `evaluateRules`, `evaluateSeriesScope`, `evaluateMusicScope`, and the `/api/lifecycle/rules/preview` route) |
| External-field rules can't broaden the database query | `evaluateGroupPreFilter` and `combinePreFilter` in `src/lib/rules/engine.ts`. The `EXTERNAL_RULE` sentinel propagates through AND/OR; pre-filter is only used when `needsFullReeval=true` |
| The empty-WHERE safety net | The block tagged `// Safety net: if all rules produced empty WHERE clauses` near the top of `evaluateRules` in `src/lib/rules/engine.ts` |
