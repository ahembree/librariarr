...
| Edited action propagates to preserved pending actions | The PUT handler in `src/app/api/lifecycle/rules/[id]/route.ts`. When `clearMatches=false` keeps existing matches and the action stays enabled, surviving `PENDING` `LifecycleAction` rows are updated to the rule set's new action config (`actionType`, `arrInstanceId`, `targetQualityProfileId`, `addImportExclusion`, `searchAfterAction`, `addArrTags`, `removeArrTags`) — they snapshot these at creation, and the executor reads `LifecycleAction.actionType`, so without this an edited action would show and run stale |
