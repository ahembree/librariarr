...
| **Skip Hardware Transcodes** | Never terminate a stream the server is encoding on a GPU, regardless of criteria (default: off) |
...
Librariarr reads the acceleration API actually in use, not the one requested — Plex reports `transcodeHwDecoding`/`transcodeHwEncoding` and can fall back to software silently, and Jellyfin/Emby report `HardwareAccelerationType`. The session detail shows the real encoder (`VAAPI`, `NVENC`, `QSV`, …) or `No (CPU)`. Neither server reports which physical device is used, only the API, so an Intel iGPU and a discrete Arc card both show `QSV` or `VAAPI`.
...
<Aside>
  **Block New Only** grandfathers whatever is playing on **each** connected server at the moment the window opens, so multi-server setups keep their in-flight streams. Because the snapshot is held in memory, restarting Librariarr mid-blackout re-grandfathers whatever is playing at that point.
</Aside>
