...
If a rule set uses Seerr fields but no Seerr instance is connected — for example after **importing** a rule set — the rule builder shows a **"Seerr instance required"** warning and disables Preview, Test Media, and Save until you connect Overseerr or Jellyseerr in Settings → Integrations. Without one, items requiring Seerr lookups would never match.
:::
...
As a safety measure, Librariarr will **not** empty or delete a collection when its rules matched items but none of them resolved to that Plex library — it leaves the collection intact and logs a warning, so a resolution problem never silently wipes a shelf. A collection is only removed when its rules genuinely match nothing.
