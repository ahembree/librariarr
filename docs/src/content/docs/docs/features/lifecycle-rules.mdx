...
<Aside type="note">
  **Repeat scheduling.** A non-destructive action (Unmonitor, Do Nothing, tag changes) leaves the item in your library, so it keeps matching the rule. To avoid scheduling the same action on every run, Librariarr won't re-schedule an item that already has a completed action of that kind — **as long as it has matched continuously**. If the item later drops out of the match set and is **re-added** (for example, deleted and then re-downloaded), it counts as a new match and a fresh action is scheduled. Completed **Delete** actions are always eligible to re-schedule — if a "deleted" item still matches, the delete likely failed and should run again.
</Aside>
