...
The value-taking operators ask a **per-source** question — `equals TMDB`
matches items that carry a TMDB id, `contains TMDB|IMDB` matches items with
either.

*Is Empty* / *Is Not Empty* take no value, so they ask about the id **list**
instead: *Is Empty* matches items with **no external ids at all**, *Is Not
Empty* matches items with at least one. Use `notEquals` / `notContains` when
you mean "has no TMDB id specifically".
:::
...
:::note[Every operator works on a list field]
The list fields — `genre`, `labels`, `country`, `arrTag`, `seerrRequestedBy`,
`watchedByUser`, `matchedByRuleSet` — all accept the same operators, and all
compare case-insensitively. *Equals* and *Contains* ask about **membership**
(*Contains* is the multi-select "any of these"), not substring: `Tag Contains
"kee"` does not match the tag `Keep`; `Tag Matches Wildcard "kee*"` does.

Until v0.29, `Matched By Rule Set` and `Requested By` quietly ignored the two
wildcard operators — a rule using one matched nothing at all, in either
direction. If you wrote such a rule and it never matched, it works now.
:::

:::note[List fields and missing *arr data]
`arrTag Is Empty` finds items that **are** in the linked *arr instance but carry
no tags. An item that isn't in *arr at all does not match it — absent *arr data
is never read as "no tags" (see [Missing *arr data is fail-closed](#external--arr-fields)
above). Use `foundInArr` for that question instead.
:::
...
subtitle track in some other language; it does not find items with no subtitles.
Use `Subtitle Language Is Empty` for that. (Otherwise `Unknown ≠ English` would
read as true and sweep in every unlabelled item.)

Track **codec** fields carry no such placeholder, so they keep the plain
reading: an item with no audio track does match `Stream Audio Codec Is Not AAC`.
:::
...
**Last Played** and **Series Last Played** reflect plays by **every** user on the media server, not just the account Librariarr connects with.

Media servers report per-item watch state for the authenticated account only (Plex's `viewCount`/`lastViewedAt`, Jellyfin/Emby's user data), so those fields are reconciled against the server-wide watch history — the same source the **Library → History** page reads — after every history sync. If a rule ever disagrees with what History shows for a title, run a sync: the reconciliation only moves a play date forward, never backward, so a rule can't fire on a series someone else in the household has been watching.

**Is Watchlisted** is different: a Plex watchlist belongs to *your* Plex account, and Librariarr reads it from plex.tv during a full sync. Adding or removing a title on Plex is therefore picked up by the next full sync rather than instantly.
:::
