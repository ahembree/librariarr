...
- **Any selection size — run in batches of 1,000** — you can select as many items as you like. The server processes at most 1,000 items per request (a safety bound: each item drives its own *arr API calls), so a larger selection is automatically split into sequential batches of up to 1,000. When your selection exceeds 1,000, the Actions bar shows the batch count (e.g. *· 3 batches*), the confirmation dialog spells it out, and the progress bar labels each pass (*Batch 2/3 · Running action*). Each batch's completed actions are recorded before the next starts, so if a run stops part-way nothing already done is lost — just re-run to finish the rest.
