...
- **Up to 1,000 items per run** — a single action targets at most 1,000 selected items. This is a safety limit: each item triggers its own *arr API calls, so an unbounded batch could run for a very long time (and, for deletions, leave a partially-applied result). If your selection is larger, the **Run action** button is disabled and the Actions bar shows the limit — narrow your filter (or deselect some items) and run it in batches.
