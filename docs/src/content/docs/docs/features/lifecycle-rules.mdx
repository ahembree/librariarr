...
An item with no such date (never requested, never approved, never declined) matches no date comparison, negated or not — use **Is Empty** to target it. If Seerr's request list can't be read in full, rule sets using Seerr fields are skipped for that run rather than evaluated against a partial list.
