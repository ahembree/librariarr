...
- **Sort order** — `Alphabetical`, `Release date`, or `Deletion date`.
...
### Deletion-date ordering across rules

With **Sort order: Deletion date**, items are arranged soonest-deletion-first. When several rules feed the collection, the ordering pools the scheduled deletion dates of **all** of them, so the shelf is correctly interleaved regardless of which rule scheduled each item. (Items without a pending deletion are placed last.)
...
- To delete a collection definition outright, select it in the dropdown and use the **trash** button. A collection can only be deleted when **no rule uses it** — the button is disabled (and the API refuses) while any rule still references it, so remove it from every rule first. Deleting it removes the collection from Plex.
