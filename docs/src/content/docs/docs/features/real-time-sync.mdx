...
- **Every way a sync can end is pushed.** Finished, failed or stopped with **Stop Sync**, every open tab updates as soon as it happens, and only after the sync's changes are visible, so a refreshed list never shows the data from before the sync. Only one background job runs at a time, so a sync you start while something else is running waits its turn — it appears as **Queued** on Settings → Servers the moment you press **Sync** (or **Sync All**). A sync you asked for goes ahead of queued background work, and a Tracearr history import that is mid-slice pauses at its next page to let it start, then carries on afterwards. Pressing **Stop Sync** on a queued sync cancels it straight away.
- **Stop Sync on a running sync takes effect at the sync's next checkpoint**: between batches of items, between libraries, or between stages. A watch-history import stops at its next page. The only step that can't be interrupted is downloading a Plex, Jellyfin or Emby server's own watch history, so a stop pressed then takes effect once that download finishes.
