...
- **A sync is announced the moment it is queued, and again when it ends.** Only one sync runs at a time, so pressing **Sync** on Settings → Servers can leave the job waiting its turn; it shows as **Pending** straight away rather than after the sync ahead of it finishes. Stopping a sync, or syncing a server that turns out to be disabled, clears the progress card immediately too.
