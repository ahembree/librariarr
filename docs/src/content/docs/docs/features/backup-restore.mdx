...
In between, every item is present with no record of ever being played. Librariarr handles this rather than letting it arm your rules: restored servers are marked as having no watch history yet, which **pauses lifecycle rules that use "Watched By User"** until a sync refills them. The pause lifts on its own.
