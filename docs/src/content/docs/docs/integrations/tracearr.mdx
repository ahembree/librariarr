...
## Lifecycle rules pause while a server's history is incomplete
...
That matters because it inverts a rule. A criterion like **Watched By User is not `alice`** matches *every* item when there are no plays to contradict it, so a rule set built to protect what the household watches would instead select the entire library, and a delete action would act on it.

So Librariarr **refuses to evaluate** rules that read **Watched By User** while any server they target has an incomplete history, rather than evaluating them against evidence it knows is missing. Concretely:
...
This is temporary and clears itself. The pause lifts the moment the server's history is complete again — when the Tracearr backfill reaches the oldest play, or when a native sync finishes its full refresh. Nothing needs to be re-armed.

<Aside type="caution" title="This does not cover Play Count and Last Played">
  The pause applies to **Watched By User**, which reads individual plays. **Play Count** and **Last Played** are separate, and they never go backwards — no sync can make an item look *less* watched than it already did, so rules reading them keep working normally throughout an import. If you want those rules held too, disable the rule set while the backfill runs.
</Aside>
...
| **A rule set is skipped with "no complete watch history yet"** | Working as intended — see [Lifecycle rules pause while a server's history is incomplete](#lifecycle-rules-pause-while-a-servers-history-is-incomplete). It clears itself when the backfill finishes. If it persists after the status line says the import is done, check that the server actually holds plays: a "complete" server with no stored history is treated as incomplete on purpose, because a restore or a data purge can empty the table without touching the flag. |
