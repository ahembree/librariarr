...
**A large history takes a while, and that is expected.** History is fetched one page at a time and Tracearr's history API returns at most 100 records per page, so a server with roughly 160,000 plays is about 1,600 sequential pages. There is nothing to press and nothing to wait on: the slices keep queueing themselves until the walk reaches the very first play, and the meantime is visible on the server's status line in Settings. While an import is actually running — a backfill slice, or the short catch-up that follows a sync — the server's card in **Settings → Media Servers** also shows an **Importing Tracearr history** panel beside the sync status, with the plays and pages imported so far in that run and, for the backfill, how far back it has reached. That is also where to look if a sync you started shows as **Queued**.
