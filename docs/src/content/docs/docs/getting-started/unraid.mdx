...
If you already run a PostgreSQL server on your network (or in another Docker container on Unraid), you can skip the `librariarr-db` container entirely and use the dedicated external database template.
...
2. **Add the external database template**

   In the Unraid Docker tab, click **Add Container** and set the **Template Repository** to:

   ```
   https://raw.githubusercontent.com/ahembree/librariarr/main/unraid/librariarr-external-db.xml
   ```
   ...
   | Field | Notes |
   |-------|-------|
   | **Database URL** | Full PostgreSQL connection string pointing to your existing database (e.g., `postgresql://librariarr:password@192.168.1.10:5432/librariarr`) |
   | **Config Path** | `/mnt/user/appdata/librariarr` (persistent storage for backups and cache) |
