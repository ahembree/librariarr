...
- Unraid, with the Community Applications plugin (or manual Docker template support)
...
See the [external database requirements](/docs/getting-started/installation/#database-requirements) for full details on supported PostgreSQL versions and privilege requirements.

<Steps>

1. **Prepare the database**

   On your existing PostgreSQL server, create a dedicated database and user for Librariarr if you haven't already:
   ...
   The user needs full privileges on the database (`CREATE`, `ALTER`, `DROP`, `INSERT`, `UPDATE`, `DELETE`). Any currently-supported PostgreSQL release works. No extensions are needed.

2. **Install only the Librariarr container**

   Follow step 2 from the [installation instructions above](#installation) using the standard Librariarr template. When configuring, update the **Database URL** to point at your existing server:

   ```
   postgresql://librariarr:your-secure-password@your-db-host:5432/librariarr
   ```

   Replace `your-db-host` with your PostgreSQL server's IP address or hostname, and update the credentials to match what you created in step 1.

3. **Verify the container is running**

   In the Docker tab, confirm `librariarr` shows as started. Navigate to `http://your-unraid-ip:3000` to access the app.

</Steps>

Librariarr creates and migrates all tables automatically on startup.
