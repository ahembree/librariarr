...
Both containers must be on the same Docker network to communicate. By default, Unraid places containers on the `bridge` network, which works out of the box.

If you use a custom Docker network, ensure both containers are on the same network so that `librariarr-db` is resolvable by hostname in the `DATABASE_URL`.

<Aside type="caution">
  If using **host** network mode, the database hostname `librariarr-db` won't resolve. Use `localhost` or `127.0.0.1` in the `DATABASE_URL` instead, and ensure PostgreSQL is configured to accept connections on the expected port.
</Aside>
...
An optional `UMASK` variable (default: `022`) is also available under advanced settings to control file permission masks.
