...
| `COOKIE_SECURE` | No | `true` in production, else `false` | Set the `Secure` attribute on the session cookie (browser only sends it over HTTPS). Defaults to `true` when `NODE_ENV=production` (the container default). Set to `false` only if you're exposing Librariarr over plain HTTP without a TLS-terminating proxy — otherwise the browser will drop the cookie and you won't stay logged in. |
