...
2. **Create a `.env` file**
   ...
   <Aside type="caution">
     `SESSION_SECRET` must be at least 32 characters. Use `openssl rand -base64 32` to generate a secure value. Changing this after deployment will invalidate all active sessions.
   </Aside>
...
| `SESSION_SECRET` | Yes | — | Random string (32+ characters) used to secure your login sessions |
