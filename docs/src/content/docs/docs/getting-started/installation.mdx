...
| `SSO_DISABLE_OVERRIDE` | No | _unset_ | Break-glass: set to `true` to force [SSO](/docs/advanced/sso/#recovery--locked-out-of-sso) off regardless of stored settings. Used to recover access when locked out of an IdP. Restart required. |
| `COOKIE_SECURE` | No | _inferred_ | Overrides the `Secure` attribute on the session cookie. When unset, the cookie is marked `Secure` whenever the request arrived over HTTPS (directly, or via a proxy sending `X-Forwarded-Proto: https`) and not otherwise, so both HTTPS-fronted and plain-HTTP LAN installs get the right answer. Set `true` to force it on (e.g. a TLS proxy that does not send `X-Forwarded-Proto`) or `false` to force it off. |
| `TRUST_PROXY_HEADERS` | No | `true` | Whether to trust `X-Forwarded-For` / `X-Real-IP` headers when computing per-client rate-limit buckets. The **last** `X-Forwarded-For` entry is used (the one your proxy appended, which the client cannot forge). Set to `false` if Librariarr is exposed directly to the internet with no proxy, where the header is entirely client-supplied. Either way, auth endpoints also enforce a global ceiling of 60 attempts per 15 minutes across all clients, so rotating the header cannot escape rate limiting. |

## Image Tags

Librariarr images are published to [Docker Hub](https://hub.docker.com/r/ahembree/librariarr):

| Tag | Description |
|-----|-------------|
| `latest` | The most recent stable release. Recommended for most users. |
| `X.Y.Z`, `X.Y`, `X` | Specific [release](https://github.com/ahembree/librariarr/releases) versions. Pin to one of these for reproducible deployments. |
| `nightly` | Rolling build from the latest commit on `main` — the newest changes that haven't been cut into a release yet. Handy for testing upcoming fixes/features, but expect occasional breakage. |
...
<Aside type="caution">
  `nightly` images may contain unreleased, unstable changes and database schema migrations that haven't shipped in a release. Take a [backup](/docs/features/backup-restore/) before trying one, and avoid `nightly` for production deployments.
</Aside>
...
- A user with full privileges on that database (`CREATE`, `ALTER`, `DROP`, `INSERT`, `UPDATE`, `DELETE`). The `CREATE` privilege is required: in addition to the application tables, Librariarr's background job queue ([Graphile Worker](https://worker.graphile.org/)) creates and manages a separate `graphile_worker` schema automatically on first start.
...
- [Security hardening](/docs/advanced/security-hardening/) — optional Docker security settings for hardened deployments
