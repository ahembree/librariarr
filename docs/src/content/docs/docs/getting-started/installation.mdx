...
| `develop` | Rolling build from the `develop` branch — the latest pre-release changes. Handy for testing upcoming features, but expect occasional breakage. |
| `develop-<sha>` | Immutable per-commit `develop` build, useful for pinning or rolling back a development deployment. |

The `latest` and version tags are published only from tagged releases on `main`. The `develop` tags are built from the separate `develop` branch and never overwrite a stable release, so switching back is just a matter of changing the tag.

<Aside type="caution">
  `develop` images may contain unreleased, unstable changes and database schema migrations that haven't shipped in a release. Take a [backup](/docs/features/backup-restore/) before trying one, and avoid `develop` for production deployments.
</Aside>
