...
The container ships with a small recovery script at `scripts/reset-auth.ts`.
Run it inside the container with `pnpm exec tsx`:

```bash
# Show current auth state (helpful first step)
docker exec -it librariarr pnpm exec tsx scripts/reset-auth.ts status

# Clear all SSO config (preserves user, Plex linkage, local password)
docker exec -it librariarr pnpm exec tsx scripts/reset-auth.ts wipe-sso

# Force-enable local login (so the form shows on the login page)
docker exec -it librariarr pnpm exec tsx scripts/reset-auth.ts enable-local

# Force-enable Plex login button
docker exec -it librariarr pnpm exec tsx scripts/reset-auth.ts enable-plex

# Last resort: delete the user. The setup screen appears on next page
# load. WARNING: this cascades to media data — synced libraries, server
# connections, and lifecycle rules are all dropped. Prefer the actions
# above when possible, or restore from a backup that has a working user.
docker exec -it librariarr pnpm exec tsx scripts/reset-auth.ts delete-user
```

`wipe-sso` is the most common recovery: clears the bad OIDC config so the
non-SSO methods (local form / Plex button) are no longer hidden by an
in-effect-but-broken SSO mode. Restart the container after running it so
the in-memory OIDC discovery cache drops.
...
```bash
docker exec librariarr pnpm exec tsx scripts/reset-auth.ts wipe-sso --force
```
