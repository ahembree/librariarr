...
<Aside>
  The provider walkthroughs below describe each IdP's current admin UI. Older
  releases may put the same settings somewhere else, or use a different config
  syntax — the Librariarr side is the same either way.
</Aside>
...
Authentik's recommended path creates the application and provider together.
The older "Providers first, then Application" path still works and is
documented as the alternative below.
...
The example below uses Authelia's **claims policies**. Older Authelia releases
predate that feature and use a slightly different client schema.
...
1. Edit Authelia's `configuration.yml`. Under `identity_providers.oidc.clients`,
   ...
   identity_providers:
     oidc:
       # Optional: customize which claims appear in the ID Token. Authelia's
...
**About Authelia's `sub` claim:** Authelia uses **UUID subject identifiers**
for every client, and there is no first-class way to swap `sub` for a
username — that's by design (the OIDC spec requires `sub` to be a stable
opaque identifier per user/client). Link by the UUID:
...
Keep oauth2-proxy on a current release — older ones have shipped
authentication-bypass and session-fixation vulnerabilities.
...
<Aside type="caution">
  Configure trusted proxy IPs at the EntryPoint level, via
  `entryPoints.<name>.forwardedHeaders.trustedIPs` — the snippets below do
  this. The older `forwardauth.trustForwardHeader=true` middleware option is
  deprecated.
</Aside>
...
<Aside type="caution">
  Keep Caddy on a current release — older ones have shipped a `forward_auth`
  vulnerability allowing identity injection.
</Aside>
...
### Header overrides — quick reference
