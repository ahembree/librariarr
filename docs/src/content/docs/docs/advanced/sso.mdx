...
# Reset the local password. Prompts interactively with no echo, or reads
# from LIBRARIARR_NEW_PASSWORD for automation. Pass --username=<name> to
# also set localUsername (required if it isn't set yet — the login form
# needs both). Bumps sessionVersion so existing sessions are invalidated.
...
docker exec -i  -e LIBRARIARR_NEW_PASSWORD=hunter2hunter2 librariarr node /tmp/reset-auth.js reset-password
