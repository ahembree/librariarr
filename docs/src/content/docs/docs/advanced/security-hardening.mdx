...
| Public API keys | Keys for the [public API](/docs/advanced/api/) are shown once and stored only as a SHA-256 hash. Each is limited to the scopes it was issued with (read-only keys cannot change anything), can expire, and stops working on the next request once deleted. Keys are sent in a header only (a key in the URL is refused), work only on `/api/v1`, are never written to backups, and are all revoked by a restore. |
